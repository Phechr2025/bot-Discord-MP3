...
async def _download_mp3_core(url: str, custom_name: Optional[str], use_cookies: bool) -> str:
    """โหลดและแปลง YouTube เป็น MP3 แล้วคืน path ไฟล์ชั่วคราวกลับมา

    use_cookies = False  -> โหมด F1 (ปกติ, ไม่ใช้ cookies.txt)
    use_cookies = True   -> โหมด F2 (ใช้ cookies.txt ช่วย bypass บางเคส)
    """
    ...
    ydl_opts = {
        ...
        "quiet": True,
        "no_warnings": True,
        # ใช้ client แบบ Android ของ YouTube แทน web ปกติ
        "extractor_args": {
            "youtube": {
                "player_client": ["android"],
            }
        },
        # จำกัดขนาดไฟล์ไม่ให้เกิน MAX_FILE_SIZE_BYTES (กันดาวน์โหลดไฟล์ใหญ่เกินส่ง Discord)
    ...
    # ถ้าเป็นโหมด F2 -> ใส่ cookiefile เพิ่ม
    if use_cookies:
...
async def download_mp3_f1(url: str, custom_name: Optional[str]) -> str:
    """โหมด F1: ใช้วิธีปกติ (ไม่ใช้ cookies.txt)"""
    return await _download_mp3_core(url, custom_name, use_cookies=False)


async def download_mp3_f2(url: str, custom_name: Optional[str]) -> str:
    """โหมด F2: ใช้ cookies.txt (สำหรับบางคลิปที่ต้องล็อกอิน/ยืนยันบอท)"""
    return await _download_mp3_core(url, custom_name, use_cookies=True)


async def download_mp3(url: str, custom_name: Optional[str]) -> str:
    """ชื่อเดิม (เผื่อมีที่ไหนเรียกใช้) ให้เท่ากับโหมด F1"""
    return await download_mp3_f1(url, custom_name)


async def send_mp3_to_dm(user: discord.User, url: str, custom_name: Optional[str], mode: str = "f1"): 
    ...
    try:
        if mode == "f2":
            mp3_path = await download_mp3_f2(url, custom_name)
        else:
            mp3_path = await download_mp3_f1(url, custom_name)
...
async def handle_form_submit(interaction: discord.Interaction, url: str, filename: str, mode: str):
