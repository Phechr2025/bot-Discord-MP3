...
# เก็บสถานะรอชื่อไฟล์ จากคำสั่ง !ytmp3 (ข้อความปกติ)
...
async def on_message(message: discord.Message):
    ...
    if user_id in PENDING_URL_BY_USER:
        ...
        await message.channel.send(
            f"กำลังโหลดครับ เดี๋ยวผมจะส่งไฟล์ให้ทาง DM 👍",
            reference=message,
        )
...
# ----------------- คำสั่งสำหรับดาวน์โหลด MP3 แบบปกติ -----------------


def _check_ytmp3_channel(channel: discord.abc.MessageableChannel) -> bool:
...
async def ytmp3_command(ctx: commands.Context, url: str):
    """คำสั่งแบบ prefix: !ytmp3 <url>"""
    if not _check_ytmp3_channel(ctx.channel):
        await ctx.reply("❌ ห้องนี้ไม่ได้รับอนุญาตให้ใช้คำสั่งดาวน์โหลด mp3", mention_author=False)
        return
...
async def ytmp3_slash(ctx: commands.Context, url: str):
    """คำสั่งแบบ /ytmp3"""
    if not _check_ytmp3_channel(ctx.channel):
        await ctx.reply("❌ ห้องนี้ไม่ได้รับอนุญาตให้ใช้คำสั่งดาวน์โหลด mp3", ephemeral=True)
        return

    # ถ้าเป็น slash จะถามชื่อไฟล์ผ่าน DM แบบสั้น ๆ
    PENDING_URL_BY_USER[ctx.author.id] = url
    await ctx.reply(
        "ผมส่ง DM ไปถามชื่อไฟล์ให้แล้วนะครับ ถ้าไม่เห็นให้เช็คว่าเปิดรับ DM จากเซิร์ฟเวอร์นี้หรือยัง",
        ephemeral=True,
    )
    ...
    try:
        ...
        await ctx.reply("❌ ไม่สามารถส่ง DM ถึงคุณได้ กรุณาเปิดรับ DM ก่อนนะครับ", ephemeral=True)
...
async def ytmp3_form(
    ...
    """สร้าง embed + ปุ่มสำหรับเรียกแบบฟอร์ม"""
    if not is_admin(ctx.author):
        await ctx.reply("❌ คำสั่งนี้ใช้ได้เฉพาะแอดมินเท่านั้น", ephemeral=True)
        return
...
async def autodel(ctx: commands.Context, mode: str):
    ...
    if mode == "on":
        ...
        await ctx.reply("🟧 ปิดระบบลบข้อความอัตโนมัติแล้ว", mention_author=False)
    else:
        await ctx.reply("❌ ให้ใช้คำว่า on หรือ off เท่านั้น", ephemeral=True)
...
async def autodel_delay(ctx: commands.Context, seconds: int):
    ...
    if seconds < 1 or seconds > 3600:
        await ctx.reply("❌ กรุณาใส่ค่า 1–3600 วินาที", ephemeral=True)
        return

    AUTO_DELETE_DELAY = seconds
    await ctx.reply(f"✅ ตั้งเวลา autodel เป็น {seconds} วินาทีแล้ว", mention_author=False)
...
async def autodel_exempt_add(ctx: commands.Context, message_id: int):
    ...
    await ctx.reply(f"✅ เพิ่มข้อความ ID `{message_id}` เข้า list ยกเว้นแล้ว", mention_author=False)
...
async def autodel_exempt_remove(ctx: commands.Context, message_id: int):
    ...
    if message_id in EXEMPT_MESSAGE_IDS:
        ...
        await ctx.reply(f"✅ ลบข้อความ ID `{message_id}` ออกจาก list ยกเว้นแล้ว", mention_author=False)
    else:
        await ctx.reply("❌ ไม่พบข้อความ ID นี้ใน list ยกเว้น", mention_author=False)
...
async def add_admin(ctx: commands.Context, user_id: int):
    if not is_owner(ctx.author):
        await ctx.reply("❌ คำสั่งนี้ใช้ได้เฉพาะ OWNER เท่านั้น", ephemeral=True)
        return

    ADMIN_IDS.add(user_id)
    await ctx.reply(f"✅ เพิ่ม `<@{user_id}>` เป็นแอดมินแล้ว", mention_author=False)
...
async def remove_admin(ctx: commands.Context, user_id: int):
    ...
    if user_id in ADMIN_IDS:
        ...
        await ctx.reply(f"✅ เอา `<@{user_id}>` ออกจากแอดมินแล้ว", mention_author=False)
    else:
        await ctx.reply("❌ ผู้ใช้นี้ไม่ได้เป็นแอดมินอยู่แล้ว", mention_author=False)
