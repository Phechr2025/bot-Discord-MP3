...
# ---------------- คำสั่ง !ytmp3 / /ytmp3 (Hybrid Command) ----------------
...
async def ytmp3(ctx: commands.Context, url: str):
    ...
    if not isinstance(ctx.channel, discord.DMChannel):
        if YTMP3_ALLOWED_CHANNEL_IDS and ctx.channel.id not in YTMP3_ALLOWED_CHANNEL_IDS:
            await ctx.reply("❌ ใช้คำสั่งนี้ได้เฉพาะในห้องที่กำหนดเท่านั้น", ephemeral=False)
            return
...
# ---------------- คำสั่งควบคุมระบบลบข้อความ (Hybrid) ----------------

@bot.hybrid_command(name="autodel", description="เปิด/ปิดระบบลบข้อความอัตโนมัติ")
...
async def autodel_error(ctx: commands.Context, error: commands.CommandError):
    if isinstance(error, commands.MissingPermissions):
        await ctx.reply("❌ คุณไม่มีสิทธิ์ใช้คำสั่งนี้ (ต้องมีสิทธิ์ Manage Messages)")


# ---------------- คำสั่งจัดการข้อความที่ยกเว้น (EXEMPT_MESSAGE_IDS) ----------------

@bot.hybrid_command(name="autodel_exempt_add", description="เพิ่มข้อความที่ไม่ต้องลบ (ตาม ID)")
@commands.has_permissions(manage_messages=True)
...
@bot.hybrid_command(name="autodel_word_add", description="เพิ่มคำสำหรับใช้กรองลบข้อความอัตโนมัติ")
@commands.has_permissions(manage_messages=True)
...
@bot.hybrid_command(name="autodel_word_remove", description="ลบคำออกจากรายการกรองลบข้อความอัตโนมัติ")
@commands.has_permissions(manage_messages=True)
...
@bot.hybrid_command(name="autodel_delay", description="ตั้งเวลา (วินาที) ที่จะลบข้อความอัตโนมัติ")
@commands.has_permissions(manage_messages=True)
